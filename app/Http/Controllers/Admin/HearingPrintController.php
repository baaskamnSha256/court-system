...
class HearingPrintController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        return view('hearings.print', [
            ...
            'searchUrl' => route('admin.hearings.print'),
            'downloadUrl' => route('admin.hearings.print.download', $request->query()),
    ...
    public function download(Request $request)
    {
        ...
        $headers = [
            ...
            'Шүүгдэгчийн нэр', 'Зүйл анги', 'Өмгөөлөгчийн нэр',
            'ТСАХ', 'Бусад',
        ];
        ...
        foreach ($hearings as $h) {
            ...
            if (count($def)) $lawyerLines[] = 'Шүүгдэгчийн өмгөөлөгч: ' . implode(', ', $def);
            if (count($victim)) $lawyerLines[] = 'Хохирогчийн өмгөөлөгч: ' . implode(', ', $victim);
            if (count($victimRep)) $lawyerLines[] = 'ХХЁТ-ийн өмгөөлөгч: ' . implode(', ', $victimRep);
            if (count($civilPl)) $lawyerLines[] = 'Иргэний нэхэмжлэгчийн өмгөөлөгч: ' . implode(', ', $civilPl);
            if (count($civilDef)) $lawyerLines[] = 'Иргэний хариуцагчийн өмгөөлөгч: ' . implode(', ', $civilDef);
            ...
            if (trim((string)($h->civil_plaintiff ?? '')) !== '') $otherLines[] = 'Иргэний нэхэмжлэгч: ' . trim($h->civil_plaintiff);
            if (trim((string)($h->civil_defendant ?? '')) !== '') $otherLines[] = 'Иргэний хариуцагч: ' . trim($h->civil_defendant);
