...
class TextMaskController extends Controller
{
    public function index()
    {
        return view('textmask.index', [
            ...
            'actionUrl' => route('admin.textmask.process'),
        ]);
    }
    ...
    public function downloadPreview(Request $request)
    {
        ...
        if ($path === '' || ! is_file($path)) {
            return redirect()->route('admin.textmask.index')->with('error', 'Урьдчилгаа файл олдсонгүй. Дахин нууцлаад урьдчилж хараарай.');
        }

        // Prevent re-download of stale path in future sessions.
    ...
    public function process(Request $request, DocxTextMasker $masker)
    {
        $data = $request->validate([
            ...
            'exclude_phrases' => ['nullable', 'string'],
            'auto_phone' => ['nullable'],
            'auto_register' => ['nullable'],
            'auto_plate' => ['nullable'],
            'auto_initial_name' => ['nullable'],
            'auto_ovogt_name' => ['nullable'],
            'mask_char' => ['nullable', 'string', 'max:1'],
            'action' => ['nullable', 'string'],
        ]);

        $phrases = preg_split('/\\r\\n|\\r|\\n/u', (string) ($data['phrases'] ?? '')) ?: [];
        $excludePhrases = preg_split('/\\r\\n|\\r|\\n/u', (string) ($data['exclude_phrases'] ?? '')) ?: [];
        $auto = [
            ...
            'ovogt_name' => (bool) ($data['auto_ovogt_name'] ?? false),
        ];
        ...
        try {
            $masker->mask($inPath, $outPath, $phrases, $auto, $maskChar, $excludePhrases);
        } catch (\Throwable $e) {
        ...
        if (($data['action'] ?? '') === 'preview') {
            ...
            return view('textmask.preview', [
                ...
                'downloadUrl' => route('admin.textmask.download'),
                'backUrl' => route('admin.textmask.index'),
            ]);
        }
