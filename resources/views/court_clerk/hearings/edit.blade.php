...
<div class="max-w-5xl mx-auto">
    ...
    <form method="POST" action="{{ route('court_clerk.hearings.update', $hearing) }}" class="bg-white border border-gray-200 rounded-2xl p-6 space-y-6">
        @csrf
        @method('PUT')
        ...
        <div x-data="judgePanelExclude({judges: @js($judges->map(fn($j)=>['id'=>(string)$j->id,'name'=>$j->name])->values()), presiding: '{{ old('presiding_judge_id', $presiding ?? '') }}', m1: '{{ old('member_judge_1_id', $m1 ?? '') }}', m2: '{{ old('member_judge_2_id', $m2 ?? '') }}'})" x-init="init()" class="space-y-4">
        ...
        <div>
            <label class="text-sm font-semibold text-gray-700">Шүүгдэгчийн нэр</label>
