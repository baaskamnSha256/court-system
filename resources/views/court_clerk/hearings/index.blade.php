...
<div class="space-y-6">
    ...
    <div class="flex flex-wrap items-center justify-between gap-4">
        ...
        <a href="{{ route('court_clerk.hearings.create') }}" class="inline-flex items-center gap-2 px-4 py-2.5 rounded-xl bg-slate-800 text-white text-sm font-medium hover:bg-slate-700 transition-colors shadow-sm">
            + Хурлын зар оруулах
        </a>
    </div>
                        ...
                        <tr class="border-b border-slate-100 last:border-0 hover:bg-slate-50/50 transition-colors">
                            ...
                            <td class="px-4 py-3 text-right">
                                <span class="inline-flex items-center gap-2">
                                    <a href="{{ route('court_clerk.hearings.edit', $h) }}" class="inline-flex items-center px-3 py-1.5 rounded-lg bg-slate-100 text-slate-700 text-sm font-medium hover:bg-slate-200 transition-colors">Засварлах</a>
                                    <form class="inline" method="POST" action="{{ route('court_clerk.hearings.destroy', $h) }}" onsubmit="return confirm('Энэ хурлын зарыг устгах уу? Зөвшөөрч байна уу?');">
                                        @csrf @method('DELETE')
                                        <button type="submit" class="inline-flex items-center px-3 py-1.5 rounded-lg text-red-600 hover:bg-red-50 text-sm font-medium transition-colors">Устгах</button>
                                    </form>
                                </span>
                            </td>
                        </tr>
